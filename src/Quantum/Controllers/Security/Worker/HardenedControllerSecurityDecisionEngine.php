...
final class HardenedControllerSecurityDecisionEngine implements ControllerSecurityDecisionEngineInterface
{
    public function __construct(
        private readonly ControllerSecurityPolicyRegistryInterface $registry,
        private readonly PolicyEvaluationSandbox $sandbox,
    ...
    public function decide(SecurityEvaluationRequest $request): SecurityDecision
    {
        ...
        foreach ($candidates as $policy) {
            ...
            if ($cached !== null) {
                ...
                [$decision, $disposition] = $this->sandboxEvaluate($policy, $request);
                $workerDispositions[] = $disposition;
        ...
        $worstDisposition = $this->sandbox->workerDispositionForRequest($workerDispositions);

        if (count($denials) > 0) {
            $denial = $denials[0];
            if ($denial->reasonCode !== 'policy_circuit_breaker_open' && ControllerWorkerDisposition::isTrustFailure($worstDisposition)) {
                $denial = new SecurityDecision(
                    effect: SecurityDecisionEffect::Deny,
                    policyId: 'security.worker_terminate',
                    reasonCode: 'policy_worker_trust_failure',
                    obligations: array_merge($denial->obligations, [
                        'worker_disposition' => ControllerWorkerDisposition::describe($worstDisposition),
                    ]),
                );
            } elseif (
                ControllerWorkerDisposition::isTerminal($worstDisposition)
                && ! isset($denial->obligations['worker_disposition'])
            ) {
                $denial = new SecurityDecision(
                    effect: SecurityDecisionEffect::Deny,
    ...
    /**
     * @return array{0: SecurityDecision, 1: int}
     */
    private function sandboxEvaluate(mixed $policy, SecurityEvaluationRequest $request): array
    {
        \assert($policy instanceof \Quantum\Controllers\Security\Contracts\ControllerSecurityPolicyInterface);
        $noop = static function (): void {};
        $result = $this->sandbox->evaluate($policy, $request, $noop, $noop);

        $decision = $result['decision'] ?? SecurityDecision::deny(
            policyId: 'security.evaluation_failed',
            reasonCode: 'sandbox_returned_invalid_payload',
            obligations: ['policy_id' => $policy->id()],
        );
        $disposition = $result['workerDisposition'] ?? ControllerWorkerDisposition::TerminateOnTrustFailure;
    ...
    {
        if (! $policy instanceof \Quantum\Controllers\Security\Policy\ControllerSecurityPolicy) {
            return [];
        }
        try {
            $noop = static function (): void {};
            $result = $this->sandbox->evaluate($policy, $request, $noop, $noop);
            return ($result['decision'] ?? null)?->obligations ?? [];
        } catch (\Throwable) {
            return [];
        }
    }
