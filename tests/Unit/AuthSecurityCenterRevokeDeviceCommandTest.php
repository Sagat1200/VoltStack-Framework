...
PHP
    ...
    public function test_it_allows_session_scope_when_distributed_guard_is_recent_lag_for_delegated_session_target(): void
    {
        $seedNow = time();
        $app = $this->bootstrappedApplication();
        $this->seedFixtures($app, $seedNow);
        ...
        self::assertSame(0, $exitCode);
        self::assertSame('recent_lag', $payload['distributed_guard']['activity_drift']['drift_profile'] ?? null);
        self::assertSame('sessions_only', $payload['distributed_guard']['operational_response']['authorization_mode_scope_policies']['delegated_admin']['remote_mutation_scope_policy'] ?? null);
        self::assertFalse($payload['distributed_guard_scope_decision']['should_deny'] ?? true);
        ...
        self::assertSame('delegated_admin_sessions_scope_target', $payload['distributed_guard_scope_decision']['actor_target_scope_relation'] ?? null);
        self::assertSame('sessions_only', $payload['distributed_guard_scope_decision']['scope_policy'] ?? null);
        self::assertSame('actor_target_scope_relation_scope_policy', $payload['distributed_guard_scope_decision']['policy_source'] ?? null);
        self::assertSame(
    ...
    public function test_it_allows_session_scope_when_distributed_guard_is_partial_visibility(): void
    {
        ...
        self::assertSame('direct_admin_sessions_scope_target', $payload['distributed_guard_scope_decision']['actor_target_scope_relation'] ?? null);
        self::assertSame('sessions_only', $payload['distributed_guard_scope_decision']['scope_policy'] ?? null);
        self::assertSame('actor_target_scope_relation_scope_policy', $payload['distributed_guard_scope_decision']['policy_source'] ?? null);
        self::assertSame('distributed_partial_visibility_guard_privileged_admin_direct_sessions_target_policy', $payload['distributed_guard_scope_decision']['policy_reason_code'] ?? null);
        self::assertSame(['sessions'], $payload['distributed_guard_scope_decision']['allowed_scopes'] ?? null);
        self::assertSame(['all', 'trusted-devices'], $payload['distributed_guard_scope_decision']['denied_scopes'] ?? null);
        self::assertNull($sessions->find('session-admin-alpha'));
        self::assertNull($sessions->find('session-admin-alpha-peer'));
    ...
    public function test_it_rejects_full_scope_when_distributed_guard_is_partial_visibility_for_delegated_actor(): void
    {
        $seedNow = time();
        $app = $this->bootstrappedApplication();
        $this->seedFixtures($app, $seedNow);
        ...
        self::assertSame('distributed_partial_visibility_guard_delegated_support_delegated_full_target_all_scope', $payload['reason_code'] ?? null);
        self::assertSame('partial_visibility', $payload['distributed_guard']['activity_drift']['drift_profile'] ?? null);
        self::assertSame('sessions_only', $payload['distributed_guard']['operational_response']['remote_mutation_scope_policy'] ?? null);
        ...
        self::assertSame('delegated_support', $payload['distributed_guard_scope_decision']['actor_privilege_level'] ?? null);
        self::assertSame('delegated_administrative_target', $payload['distributed_guard_scope_decision']['actor_target_relation'] ?? null);
        self::assertSame('delegated_admin_full_scope_target', $payload['distributed_guard_scope_decision']['actor_target_scope_relation'] ?? null);
        self::assertSame('deny_all', $payload['distributed_guard_scope_decision']['scope_policy'] ?? null);
        self::assertSame('actor_target_scope_relation_scope_policy', $payload['distributed_guard_scope_decision']['policy_source'] ?? null);
        self::assertSame('distributed_partial_visibility_guard_delegated_support_delegated_full_target_policy', $payload['distributed_guard_scope_decision']['policy_reason_code'] ?? null);
    ...
    public function test_it_rejects_full_scope_when_distributed_guard_is_recent_lag_for_delegated_full_target(): void
    {
        ...
        $app = $this->bootstrappedApplication();
        $this->seedFixtures($app, $seedNow);
        $auditLogPath = $this->basePath . DIRECTORY_SEPARATOR . 'var' . DIRECTORY_SEPARATOR . 'audit' . DIRECTORY_SEPARATOR . 'security-center-recent-lag-delegated-full-target.jsonl';
        $auditLogSource = $this->basePath . DIRECTORY_SEPARATOR . 'var' . DIRECTORY_SEPARATOR . 'audit' . DIRECTORY_SEPARATOR . 'security-center-longitudinal.jsonl';
        $this->seedLongitudinalAuditTrail($auditLogSource, $seedNow);
        ...
        self::assertSame(1, $exitCode);
        self::assertSame('distributed_recent_lag_guard_delegated_support_delegated_full_target_all_scope', $payload['reason_code'] ?? null);
        self::assertSame('recent_lag', $payload['distributed_guard']['activity_drift']['drift_profile'] ?? null);
        self::assertTrue($payload['distributed_guard_scope_decision']['should_deny'] ?? false);
        self::assertSame('all', $payload['distributed_guard_scope_decision']['scope'] ?? null);
        self::assertSame('delegated_admin', $payload['distributed_guard_scope_decision']['authorization_mode'] ?? null);
        ...
        self::assertSame(['all', 'trusted-devices'], $payload['distributed_guard_scope_decision']['denied_scopes'] ?? null);
        self::assertSame(
            'distributed_recent_lag_guard_delegated_support_delegated_full_target_all_scope',
            $payload['distributed_guard_scope_decision']['reason_code'] ?? null,
        );
        self::assertInstanceOf(AuthenticationSession::class, $sessions->find('session-admin-alpha'));
        self::assertInstanceOf(AuthenticationSession::class, $sessions->find('session-admin-alpha-peer'));
        ...
        self::assertSame('security_center_device_revocation_rejected', $events[0]['event'] ?? null);
        self::assertSame('distributed_guard_denied', $events[0]['result'] ?? null);
        self::assertSame('distributed_recent_lag_guard_delegated_support_delegated_full_target_all_scope', $events[0]['reason_code'] ?? null);
        self::assertSame(
            'distributed_recent_lag_guard_delegated_support_delegated_full_target_policy',
    ...
    public function test_it_rejects_trusted_device_scope_when_distributed_guard_is_recent_lag_for_delegated_actor(): void
    {
        $seedNow = time();
        $app = $this->bootstrappedApplication();
        $this->seedFixtures($app, $seedNow);
        $auditLogPath = $this->basePath . DIRECTORY_SEPARATOR . 'var' . DIRECTORY_SEPARATOR . 'audit' . DIRECTORY_SEPARATOR . 'security-center-recent-lag-delegated-guard.jsonl';
        $auditLogSource = $this->basePath . DIRECTORY_SEPARATOR . 'var' . DIRECTORY_SEPARATOR . 'audit' . DIRECTORY_SEPARATOR . 'security-center-longitudinal.jsonl';
        $this->seedLongitudinalAuditTrail($auditLogSource, $seedNow);
        ...
        self::assertSame(1, $exitCode);
        self::assertSame('distributed_recent_lag_guard_delegated_support_delegated_trusted_target_trusted_devices_scope', $payload['reason_code'] ?? null);
        self::assertSame('recent_lag', $payload['distributed_guard']['activity_drift']['drift_profile'] ?? null);
        self::assertSame('observe_recent_lag', $payload['distributed_guard']['operational_response']['response_mode'] ?? null);
        self::assertTrue($payload['distributed_guard_scope_decision']['should_deny'] ?? false);
        ...
        self::assertSame('delegated_support', $payload['distributed_guard_scope_decision']['actor_privilege_level'] ?? null);
        self::assertSame('delegated_administrative_target', $payload['distributed_guard_scope_decision']['actor_target_relation'] ?? null);
        self::assertSame('delegated_admin_trusted_devices_scope_target', $payload['distributed_guard_scope_decision']['actor_target_scope_relation'] ?? null);
        self::assertSame('deny_all', $payload['distributed_guard_scope_decision']['scope_policy'] ?? null);
        self::assertSame('actor_target_scope_relation_scope_policy', $payload['distributed_guard_scope_decision']['policy_source'] ?? null);
        self::assertSame(
            'distributed_recent_lag_guard_delegated_support_delegated_trusted_target_policy',
            $payload['distributed_guard_scope_decision']['policy_reason_code'] ?? null,
        );
        self::assertSame([], $payload['distributed_guard_scope_decision']['allowed_scopes'] ?? null);
        self::assertSame(['all', 'sessions', 'trusted-devices'], $payload['distributed_guard_scope_decision']['denied_scopes'] ?? null);
        self::assertSame(
            'distributed_recent_lag_guard_delegated_support_delegated_trusted_target_trusted_devices_scope',
            $payload['distributed_guard_scope_decision']['reason_code'] ?? null,
        );
        self::assertInstanceOf(AuthenticationSession::class, $sessions->find('session-admin-alpha'));
        self::assertInstanceOf(AuthenticationSession::class, $sessions->find('session-admin-alpha-peer'));
        ...
        self::assertSame('security_center_device_revocation_rejected', $events[0]['event'] ?? null);
        self::assertSame('distributed_guard_denied', $events[0]['result'] ?? null);
        self::assertSame('distributed_recent_lag_guard_delegated_support_delegated_trusted_target_trusted_devices_scope', $events[0]['reason_code'] ?? null);
        self::assertSame(
            'distributed_recent_lag_guard_delegated_support_delegated_trusted_target_policy',
            $events[0]['distributed_guard_scope_decision']['policy_reason_code'] ?? null,
        );
        self::assertSame('delegated_admin_trusted_devices_scope_target', $events[0]['distributed_guard_scope_decision']['actor_target_scope_relation'] ?? null);
    }

    public function test_it_rejects_trusted_device_scope_when_distributed_guard_is_recent_lag_for_self_governed_delegated_actor(): void
    {
        $seedNow = time();
        $app = $this->bootstrappedApplication();
        $this->seedFixtures($app, $seedNow);
        $auditLogPath = $this->basePath . DIRECTORY_SEPARATOR . 'var' . DIRECTORY_SEPARATOR . 'audit' . DIRECTORY_SEPARATOR . 'security-center-recent-lag-self-governed-delegated.jsonl';
        $auditLogSource = $this->basePath . DIRECTORY_SEPARATOR . 'var' . DIRECTORY_SEPARATOR . 'audit' . DIRECTORY_SEPARATOR . 'security-center-longitudinal.jsonl';
        $this->seedLongitudinalAuditTrail($auditLogSource, $seedNow);
        ...
        self::assertSame(1, $exitCode);
        self::assertSame(
            'distributed_recent_lag_guard_delegated_support_self_governed_trusted_devices_scope',
        ...
        self::assertSame('self_governed_trusted_devices_scope_target', $payload['distributed_guard_scope_decision']['actor_target_scope_relation'] ?? null);
        self::assertSame('sessions_only', $payload['distributed_guard_scope_decision']['scope_policy'] ?? null);
        self::assertSame('actor_target_relation_scope_policy', $payload['distributed_guard_scope_decision']['policy_source'] ?? null);
        self::assertSame(
            'distributed_recent_lag_guard_delegated_support_self_governed_policy',
            $payload['distributed_guard_scope_decision']['policy_reason_code'] ?? null,
        );
        self::assertSame(['sessions'], $payload['distributed_guard_scope_decision']['allowed_scopes'] ?? null);
        self::assertSame(['all', 'trusted-devices'], $payload['distributed_guard_scope_decision']['denied_scopes'] ?? null);
        ...
        self::assertSame(
            'distributed_recent_lag_guard_delegated_support_self_governed_trusted_devices_scope',
            $events[0]['reason_code'] ?? null,
        );
        self::assertSame(
            'distributed_recent_lag_guard_delegated_support_self_governed_policy',
            $events[0]['distributed_guard_scope_decision']['policy_reason_code'] ?? null,
        );
        self::assertSame('self_governed_trusted_devices_scope_target', $events[0]['distributed_guard_scope_decision']['actor_target_scope_relation'] ?? null);
    }

    public function test_it_allows_trusted_device_scope_when_distributed_guard_is_concentrated_activity_for_self_governed_delegated_actor(): void
    {
        $seedNow = time();
        $app = $this->bootstrappedApplication();
        $this->seedFixtures($app, $seedNow);
        $auditLogSource = $this->basePath . DIRECTORY_SEPARATOR . 'var' . DIRECTORY_SEPARATOR . 'audit' . DIRECTORY_SEPARATOR . 'security-center-concentrated.jsonl';
        $this->seedLongitudinalConcentratedAuditTrail($auditLogSource, $seedNow);
        ...
        self::assertSame(0, $exitCode);
        self::assertSame('concentrated_activity', $payload['distributed_guard']['activity_drift']['drift_profile'] ?? null);
        self::assertSame('concentrated', $payload['distributed_guard']['activity_drift']['coordination_profile'] ?? null);
        self::assertSame('monitor_concentrated_activity', $payload['distributed_guard']['activity_drift']['recommended_action'] ?? null);
        self::assertSame('observe_concentrated_activity', $payload['distributed_guard']['operational_response']['response_mode'] ?? null);
        self::assertSame(
            [
            ...
            $payload['distributed_guard']['operational_response']['degraded_scope_profiles'] ?? null,
        );
        self::assertFalse($payload['distributed_guard_scope_decision']['should_deny'] ?? true);
        self::assertSame('trusted-devices', $payload['distributed_guard_scope_decision']['scope'] ?? null);
        self::assertSame('delegated_admin', $payload['distributed_guard_scope_decision']['authorization_mode'] ?? null);
        ...
        self::assertSame([], $payload['distributed_guard_scope_decision']['denied_scopes'] ?? null);
        self::assertInstanceOf(AuthenticationSession::class, $sessions->find('session-support-admin'));
        self::assertNull($trustedDevices->find('tdv_support_admin'));
    }

    public function test_it_rejects_full_scope_when_distributed_guard_is_concentrated_activity_for_delegated_target(): void
    {
        $seedNow = time();
        $app = $this->bootstrappedApplication();
        $this->seedFixtures($app, $seedNow);
        $auditLogPath = $this->basePath . DIRECTORY_SEPARATOR . 'var' . DIRECTORY_SEPARATOR . 'audit' . DIRECTORY_SEPARATOR . 'security-center-concentrated-delegated-guard.jsonl';
        $auditLogSource = $this->basePath . DIRECTORY_SEPARATOR . 'var' . DIRECTORY_SEPARATOR . 'audit' . DIRECTORY_SEPARATOR . 'security-center-concentrated.jsonl';
        $this->seedLongitudinalConcentratedAuditTrail($auditLogSource, $seedNow);
        ...
        self::assertSame(1, $exitCode);
        self::assertSame('distributed_concentrated_activity_guard_delegated_support_delegated_full_target_all_scope', $payload['reason_code'] ?? null);
        self::assertSame('concentrated_activity', $payload['distributed_guard']['activity_drift']['drift_profile'] ?? null);
        self::assertSame('observe_concentrated_activity', $payload['distributed_guard']['operational_response']['response_mode'] ?? null);
        self::assertTrue($payload['distributed_guard_scope_decision']['should_deny'] ?? false);
        self::assertSame('all', $payload['distributed_guard_scope_decision']['scope'] ?? null);
        ...
        self::assertSame(['all', 'trusted-devices'], $payload['distributed_guard_scope_decision']['denied_scopes'] ?? null);
        self::assertSame(
            'distributed_concentrated_activity_guard_delegated_support_delegated_full_target_all_scope',
            $payload['distributed_guard_scope_decision']['reason_code'] ?? null,
        );
        self::assertInstanceOf(AuthenticationSession::class, $sessions->find('session-admin-alpha'));
        self::assertInstanceOf(AuthenticationSession::class, $sessions->find('session-admin-alpha-peer'));
        ...
        self::assertSame('security_center_device_revocation_rejected', $events[0]['event'] ?? null);
        self::assertSame('distributed_guard_denied', $events[0]['result'] ?? null);
        self::assertSame('distributed_concentrated_activity_guard_delegated_support_delegated_full_target_all_scope', $events[0]['reason_code'] ?? null);
        self::assertSame(
            'distributed_concentrated_activity_guard_delegated_support_delegated_full_target_policy',
            $events[0]['distributed_guard_scope_decision']['policy_reason_code'] ?? null,
        );
    ...
    public function test_it_allows_session_scope_when_distributed_guard_is_partial_visibility_for_delegated_session_target(): void
    {
        $seedNow = time();
        $app = $this->bootstrappedApplication();
        $this->seedFixtures($app, $seedNow);
        ...
        self::assertSame('actor_target_scope_relation_scope_policy', $payload['distributed_guard_scope_decision']['policy_source'] ?? null);
        self::assertSame(
            'distributed_partial_visibility_guard_delegated_support_delegated_sessions_target_policy',
            $payload['distributed_guard_scope_decision']['policy_reason_code'] ?? null,
        );
        self::assertSame(['sessions'], $payload['distributed_guard_scope_decision']['allowed_scopes'] ?? null);
        self::assertSame(['all', 'trusted-devices'], $payload['distributed_guard_scope_decision']['denied_scopes'] ?? null);
    ...
    public function test_it_rejects_trusted_device_scope_when_distributed_guard_is_partial_visibility(): void
    {
        ...
        self::assertSame(1, $exitCode);
        self::assertSame('distributed_partial_visibility_guard_privileged_admin_direct_trusted_target_trusted_devices_scope', $payload['reason_code'] ?? null);
        self::assertSame('partial_visibility', $payload['distributed_guard']['activity_drift']['drift_profile'] ?? null);
        self::assertSame('sessions_only', $payload['distributed_guard']['operational_response']['remote_mutation_scope_policy'] ?? null);
        self::assertTrue($payload['distributed_guard_scope_decision']['should_deny'] ?? false);
        self::assertSame('trusted-devices', $payload['distributed_guard_scope_decision']['scope'] ?? null);
        ...
        self::assertSame('direct_admin', $payload['distributed_guard_scope_decision']['authorization_mode'] ?? null);
        self::assertSame('direct_administrative_target', $payload['distributed_guard_scope_decision']['actor_target_relation'] ?? null);
        self::assertSame('direct_admin_trusted_devices_scope_target', $payload['distributed_guard_scope_decision']['actor_target_scope_relation'] ?? null);
        self::assertSame('deny_all', $payload['distributed_guard_scope_decision']['scope_policy'] ?? null);
        self::assertSame('actor_target_scope_relation_scope_policy', $payload['distributed_guard_scope_decision']['policy_source'] ?? null);
        self::assertSame('distributed_partial_visibility_guard_privileged_admin_direct_trusted_target_policy', $payload['distributed_guard_scope_decision']['policy_reason_code'] ?? null);
        self::assertSame('distributed_partial_visibility_guard_privileged_admin_direct_trusted_target_trusted_devices_scope', $payload['distributed_guard_scope_decision']['reason_code'] ?? null);
        self::assertInstanceOf(AuthenticationSession::class, $sessions->find('session-admin-alpha'));
        self::assertInstanceOf(AuthenticationSession::class, $sessions->find('session-admin-alpha-peer'));
        self::assertInstanceOf(TrustedDevice::class, $trustedDevices->find('tdv_admin_alpha'));
        ...
        self::assertSame('security_center_device_revocation_rejected', $events[0]['event'] ?? null);
        self::assertSame('distributed_guard_denied', $events[0]['result'] ?? null);
        self::assertSame('distributed_partial_visibility_guard_privileged_admin_direct_trusted_target_trusted_devices_scope', $events[0]['reason_code'] ?? null);
        self::assertSame('distributed_partial_visibility_guard_privileged_admin_direct_trusted_target_policy', $events[0]['distributed_guard_scope_decision']['policy_reason_code'] ?? null);
        self::assertSame('distributed_partial_visibility_guard_privileged_admin_direct_trusted_target_trusted_devices_scope', $events[0]['distributed_guard_scope_decision']['reason_code'] ?? null);
    }

    public function test_it_allows_trusted_device_scope_when_distributed_guard_is_recent_lag_for_direct_target(): void
    {
        $seedNow = time();
        $app = $this->bootstrappedApplication();
        $this->seedFixtures($app, $seedNow);
        ...
        self::assertSame('direct_administrative_target', $payload['distributed_guard_scope_decision']['actor_target_relation'] ?? null);
        self::assertSame('direct_admin_trusted_devices_scope_target', $payload['distributed_guard_scope_decision']['actor_target_scope_relation'] ?? null);
        self::assertSame('trusted_devices_only', $payload['distributed_guard_scope_decision']['scope_policy'] ?? null);
        self::assertSame('actor_target_scope_relation_scope_policy', $payload['distributed_guard_scope_decision']['policy_source'] ?? null);
        self::assertSame('distributed_recent_lag_guard_direct_trusted_target_policy', $payload['distributed_guard_scope_decision']['policy_reason_code'] ?? null);
        self::assertSame(['trusted-devices'], $payload['distributed_guard_scope_decision']['allowed_scopes'] ?? null);
        self::assertSame(['all', 'sessions'], $payload['distributed_guard_scope_decision']['denied_scopes'] ?? null);
        self::assertInstanceOf(AuthenticationSession::class, $sessions->find('session-admin-alpha'));
        self::assertInstanceOf(AuthenticationSession::class, $sessions->find('session-admin-alpha-peer'));
        self::assertNull($trustedDevices->find('tdv_admin_alpha'));
    }

    public function test_it_allows_session_scope_when_distributed_guard_is_concentrated_activity_for_direct_target(): void
    {
        $seedNow = time();
        $app = $this->bootstrappedApplication();
        $this->seedFixtures($app, $seedNow);
        ...
        self::assertSame('sessions_only', $payload['distributed_guard_scope_decision']['scope_policy'] ?? null);
        self::assertSame('actor_target_scope_relation_scope_policy', $payload['distributed_guard_scope_decision']['policy_source'] ?? null);
        self::assertSame('distributed_concentrated_activity_guard_direct_sessions_target_policy', $payload['distributed_guard_scope_decision']['policy_reason_code'] ?? null);
        self::assertSame(['sessions'], $payload['distributed_guard_scope_decision']['allowed_scopes'] ?? null);
        self::assertSame(['all', 'trusted-devices'], $payload['distributed_guard_scope_decision']['denied_scopes'] ?? null);
        self::assertNull($sessions->find('session-admin-alpha'));
        self::assertNull($sessions->find('session-admin-alpha-peer'));
