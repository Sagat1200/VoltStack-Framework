...
final class AuthManagerTest extends TestCase
{
    ...
    public function test_auth_manager_supports_self_governed_full_scope_for_managed_devices(): void
    {
        $app = new Application(sys_get_temp_dir());
        $app->make(ConfigRepository::class)->set('auth.providers.local.identities', [
            [
                'id' => 291,
                'identifier' => 'self-governed-full@example.com',
                'password_hash' => password_hash('secret-123', PASSWORD_DEFAULT),
                'mfa_code' => '654321',
                'type' => 'user',
                'auth_management_authority' => 'administrative_actor',
        ...
        $router = $app->make(Router::class);
        $router->post('/self-governed-full-login', fn (): array => ['ok' => auth()->attempt([
            'identifier' => 'self-governed-full@example.com',
            'password' => 'secret-123',
            'second_factor' => '654321',
        ])]);
        $router->post('/self-governed-full-enroll', fn (): array => ['trusted' => auth()->trustCurrentDevice()]);
        $router->get('/self-governed-full-devices', function (): array {
            return [
                'devices' => array_map(static fn ($device): array => [
                    'device_reference' => $device->deviceReference,
                    'management_actor_can_manage_sessions' => $device->managementActorCanManageSessions,
                    ...
                    'management_actor_target_scope_reason_code' => $device->managementActorTargetScopeReasonCode,
                    'management_target_matches_current_identity' => $device->managementTargetMatchesCurrentIdentity,
                ], auth()->managedDevices('291')),
            ];
        });
        $router->post('/self-governed-full-revoke', function (): array {
            $request = RuntimeContext::current()?->request();
        ...
        $router->get('/self-governed-full-protected', fn (): array => ['check' => auth()->check()])->middleware('auth');

        $kernel = $app->make(HttpKernel::class);
        $login = $kernel->handle(Request::create('/self-governed-full-login', 'POST', server: [
            'HTTP_USER_AGENT' => 'Mozilla/5.0 (Macintosh; Intel Mac OS X 14_6) Safari/605.1.15',
            'HTTP_ACCEPT_LANGUAGE' => 'en-US,en;q=0.9',
            'REMOTE_ADDR' => '203.0.113.182',
        ]));
        $sessionId = $login->headers()['X-Auth-Session'] ?? null;
        self::assertIsString($sessionId);

        $enroll = $kernel->handle(Request::create(
            '/self-governed-full-enroll',
            'POST',
            cookies: [AuthenticationHttpState::SESSION_COOKIE_NAME => $sessionId],
            server: [
                'HTTP_USER_AGENT' => 'Mozilla/5.0 (Macintosh; Intel Mac OS X 14_6) Safari/605.1.15',
        ...
        self::assertSame('self_governed_full_scope_target', $device['management_actor_target_scope_relation'] ?? null);
        self::assertSame('self_governed_full_scope_target', $device['management_actor_target_scope_reason_code'] ?? null);
        self::assertTrue((bool) ($device['management_target_matches_current_identity'] ?? false));

        $revokeAll = $kernel->handle(Request::create(
            '/self-governed-full-revoke',
            'POST',
            ['identity' => '291', 'device_reference' => $deviceReference, 'scope' => 'all'],
            cookies: [AuthenticationHttpState::SESSION_COOKIE_NAME => $sessionId],
            server: [
                'HTTP_ACCEPT' => 'application/json',
                'HTTP_USER_AGENT' => 'Mozilla/5.0 (Macintosh; Intel Mac OS X 14_6) Safari/605.1.15',
        ...
        $revokeAllPayload = json_decode($revokeAll->content(), true, 512, JSON_THROW_ON_ERROR);
        self::assertTrue((bool) ($revokeAllPayload['revoked'] ?? false));
        ...
        self::assertCount(0, array_filter(
            $trustedDeviceRepository->all(),
            static fn ($trustedDevice): bool => $trustedDevice->reference->identifier->value === '291'
                && $trustedDevice->deviceReference === $deviceReference,
        ));

        $protectedResponse = $kernel->handle(Request::create(
            '/self-governed-full-protected',
            'GET',
            cookies: [AuthenticationHttpState::SESSION_COOKIE_NAME => $sessionId],
            server: [
                'HTTP_ACCEPT' => 'application/json',
