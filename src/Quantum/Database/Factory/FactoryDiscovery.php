...
final class FactoryDiscovery
{
     ...
     * @param list<class-string<FactoryInterface>> $classes
     */
    ...
    public function discover(): array
    {
        ...
        foreach ($factories as $factory) {
            ...
            if ($name === '') {
                throw new \RuntimeException(sprintf('Factory [%s] must declare a non-empty name.', $factory::class));
            }

            if (isset($unique[$name])) {
                throw new \RuntimeException(sprintf(
                    'Duplicate factory name [%s] found in [%s] and [%s].',
                    $name,
                    $unique[$name]::class,
                    $factory::class,
                ));
    ...
    /**
     * @param class-string<FactoryInterface> $class
     */
    private function makeFactory(string $class): FactoryInterface
    {
        if (!class_exists($class)) {
            throw new \RuntimeException(sprintf('Configured factory class [%s] was not found.', $class));
        }

        if (!is_a($class, FactoryInterface::class, true)) {
            throw new \RuntimeException(sprintf('Configured factory [%s] must implement [%s].', $class, FactoryInterface::class));
        }
