...
final class SeederDiscovery
{
     ...
     * @param list<class-string<SeederInterface>> $classes
     */
    ...
    public function discover(): array
    {
        ...
        foreach ($seeders as $seeder) {
            $name = trim($seeder->name());
            if ($name === '') {
                throw new \RuntimeException(sprintf('Seeder [%s] must declare a non-empty name.', $seeder::class));
            }

            if (isset($unique[$name])) {
                throw new \RuntimeException(sprintf(
                    'Duplicate seeder name [%s] found in [%s] and [%s].',
                    $name,
                    $unique[$name]::class,
                    $seeder::class,
                ));
     ...
     */
    private function makeSeeder(string $class): SeederInterface
    {
        if (!class_exists($class)) {
            throw new \RuntimeException(sprintf('Configured seeder class [%s] was not found.', $class));
        }

        if (!is_a($class, SeederInterface::class, true)) {
            throw new \RuntimeException(sprintf('Configured seeder [%s] must implement [%s].', $class, SeederInterface::class));
        }
